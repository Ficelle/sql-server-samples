...
# Start Cleanup
Invoke-Sqlcmd -ServerInstance $SqlServer -Database $dbName -Query "delete from [LoanStatsPredictions];delete from Runtimestats;checkpoint;"

Write-Host "Starting parallel jobs to score " $count "loans" -ForegroundColor Yellow

while ($EndCtr -le $FinalCount)
{
    $SqlScript = [ScriptBlock]::Create("Invoke-Sqlcmd -ServerInstance `"" + $SqlServer + "`" -Query `"EXEC [dbo].[ScoreLoans] " + $StartCtr + "," + $EndCtr + "`" -Database `"$dbName`"")
