...
# Start Cleanup
Invoke-Sqlcmd -ServerInstance $SqlServer -Database $dbName -Query "delete from [LoanPredictionsWhatIf];delete from Runtimestats;checkpoint;"
...
Invoke-Sqlcmd -ServerInstance $SqlServer -Database $dbName -Query $SqlScript

Write-Host "Starting parallel jobs to score " $count "loans" -ForegroundColor Yellow

while ($EndCtr -le $FinalCount)
{
    $SqlScript = [ScriptBlock]::Create("Invoke-Sqlcmd -ServerInstance `"" + $SqlServer + "`" -Query `"EXEC [dbo].[ScoreLoansWhatIf] " + $StartCtr + "," + $EndCtr + "," + $IntRate + "`" -Database `"$dbName`"")
...
# Find out duration
$duration = Invoke-Sqlcmd -ServerInstance $SqlServer -Database $dbName -Query "select DATEDIFF(s,MIN (Runtime), MAX(Runtime)) as RuntimeSeconds from dbo.RuntimeStats;"
