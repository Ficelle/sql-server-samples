...
SSMSから[Create Database.sql]()を実行し、データベースオブジェクトを作成します。
...
PowerShellから[ImportCSVData.ps1]()を実行し、サンプルデータをインポートします。
...
[Create Columnstore Index.sql]()を実行し、機械学習による前処理の効率化のためにインポートデータをメモリ最適化列ストア構造に変換します。
...
[Create Model.sql]()を実行し、Rにデータをロードしディシジョンフォレストによるモデルトレーニングを実行し、トレーニング済みモデルをmodelテーブルに格納します。
...
[ScoreLoans.ps1]()を実行し、貸付金利を現状維持した場合の貸付評価のスコアリングを行います。
...
[WhatIf.ps1]()を実行し、貸付金利を現状維持した場合の貸付評価のスコアリングを行います。
...
3. [State Codes.xlsx]()（州名と略称のマッピング）のファイルパスを適宜変更する
